...
#define PRODUCT_NAME    "PowerReset"
#define DEVICE_NUMBER   1        // lub np. 118 jeśli chcesz mieć końcówkę IP/MAC

int telnetPort = 23; 

#include "about_html.h"  //strona o programie do SPIFFS
...
//----------------------------------------------------------
#define W5500_CS 5
byte mac[] = { 0x02, 0x50, 0x57, 0x01, 0x01, 0x75 }; //0x02,0x50,0x57,0x01,0x01,0x76 - 118,   0x75 -117
...
void sendLogPage_GET(EthernetClient &client) {
  ...
  // Rozmiar tworzonego logu
  client.println("<div style='margin-bottom:7px;font-size:0.99em;'>");
client.println("Rozmiar pliku <b>log.txt</b>: <span id='logFileSize' style='color:#ffb74d;'>…</span>");
client.println("</div>");
//
...
client.println("function highlight(line){"
  "if(line.includes('[ERROR]')||line.includes('[BŁĄD]'))return'<span style=\"color:#ff5252;font-weight:bold;\">'+line+'</span>';"
  "if(line.includes('[WARN]'))return'<span style=\"color:#ffc107;font-weight:bold;\">'+line+'</span>';"
  "if(line.includes('[INFO]'))return'<span style=\"color:#90caf9;\">'+line+'</span>';"
  "return line;"
"}");
client.println("function loadLogs(){"
  "var xhr=new XMLHttpRequest();xhr.open('GET','/api/logs',true);"
  "xhr.onload=function(){"
  "if(xhr.status==200){"
  "let lines=xhr.responseText.split('\\n');"
  "let filter=document.getElementById('logSearch').value.toLowerCase();"
  "let infoOk=document.getElementById('infoCb').checked;"
  "let warnOk=document.getElementById('warnCb').checked;"
  "let errOk=document.getElementById('errCb').checked;"
  "let html='';"
  "lines.reverse().forEach(function(line){"
    "let l=line.toLowerCase();"
    "if(l){"
    "if((l.includes('[info]')&&!infoOk)||(l.includes('[warn]')&&!warnOk)||((l.includes('[error]')||l.includes('[błąd]'))&&!errOk))return;"
    "if(filter&&l.indexOf(filter)==-1)return;"
    "html+=highlight(line)+'<br>';"
    "}"
  "});"
  ...
  "};xhr.send();"
"}");
client.println("function updateLogFileSize(){"
  "var xhr=new XMLHttpRequest();"
  "xhr.open('GET','/api/log_size',true);"
  "xhr.onload=function(){"
    "if(xhr.status==200){"
      "let s=parseInt(xhr.responseText);"
      "let txt=s>1024*1024?(s/1024/1024).toFixed(2)+' MB':s>1024?(s/1024).toFixed(1)+' kB':s+' B';"
      "document.getElementById('logFileSize').textContent=txt;"
    "}"
  "};"
  "xhr.send();"
"}");
client.println("setInterval(function(){if(autoRefresh)loadLogs();updateLogFileSize();},4000);");
...
client.println("window.addEventListener('DOMContentLoaded',function(){loadLogs();updateLogFileSize();});");
client.println("</script>");
...
void sendInputsPage_GET(EthernetClient &client) {
  sendCommonHtmlHeader(client, "");
  sendMainContainerBegin(client, "");

  // Informacja o auto-refresh
  //client.println("<p style='color:#bdbdbd; margin-bottom-right:18px;'>Strona odświeża się automatycznie co 2 sekund.</p>");
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // Kolumna: Stan
    client.print("<td><span id='inputState" + String(i) + "' style='color:" + String(state == HIGH ? "#76ff03" : "#ff5252") + "; font-weight:bold;'>");
...
function updateInputsTable() {
  fetch('/api/mainStatus')
    .then(resp => resp.json())
    .then(data => {
      for (let i = 0; i < 4; ++i) {
        let span = document.getElementById('inputState' + i);
        if (span) {
          if (data['in'+i] == 1) {
            span.innerText = "HIGH";
...
setInterval(updateInputsTable, 2000);
window.onload = updateInputsTable;
</script>
)rawliteral");
...
void sendToggleRelayPairConfirmPage(EthernetClient &client, int pairIndex) {
  sendCommonHtmlHeader(client, "Potwierdzenie przełączenia pary przekaźników");
  sendMainContainerBegin(client, "Potwierdzenie przełączenia pary przekaźników");
  int idx1 = pairIndex * 2;
  int idx2 = idx1 + 1;
  client.print("<p>Czy na pewno chcesz przełączyć PRZEKAŹNIKI nr: ");
  client.print(idx1);
  client.print(" i ");
  client.print(idx2);
  client.println("?</p>");
  client.println("<div style='margin-top:25px;'>");
  client.print("<button onclick=\"location.href='/toggle_confirm?pair=");
  client.print(pairIndex);
  client.println("'\" style='padding:10px 20px; background:#2196f3; color:#fff; border:none; border-radius:4px; cursor:pointer; margin-right:10px;'>Tak, przełącz oba</button>");
  client.println("<button onclick=\"location.href='/'\" style='padding:10px 20px; background:#616161; color:#fff; border:none; border-radius:4px; cursor:pointer;'>Anuluj</button>");
  client.println("</div>");
  client.println("</div>");
  sendCommonHtmlFooter(client);
...
//   if (idx == -1) {
//     // Brak danych – ale NIE logujemy każdej próby odświeżenia!
//     return false;
//   }
...
//   // Logujemy TYLKO pierwsze udane logowanie po starcie (brak wcześniejszego wasLoggedIn)
//   if (success && !wasLoggedIn) {
//     wasLoggedIn = true;
//     lastLoginTime = getCurrentTimeString();
//     lastLoginIP = client.remoteIP().toString();
//     logLoginAttemptToFile(client.remoteIP().toString(), true);
//   }
//   // Nieudane próby logowania tylko przy wejściu na login – nie każda autoryzacja!
//   return success;
// }
bool checkAuth(const String &headers, EthernetClient &client) {
  int idx = headers.indexOf("Authorization: Basic ");
...
// Wymuszona autoryzacja – redirect do logowania
...
// Wylogowanie – kasuj ciasteczko i redirect na login
void handleLogout(EthernetClient &client, const String& headers) {
    String sid = getSessionIdFromCookie(headers);
    if (sid.length()) sessionMap.erase(sid);
    client.println("HTTP/1.1 302 Found");
    client.println("Set-Cookie: SESSIONID=; Path=/; Max-Age=0");
    client.println("Location: /login");
    client.println("Connection: close");
    client.println();
}
...
void displayWelcomeScreen() {
  ...
  String title = "PowerReset " + firmwareVersion;
...
void displayOnTFT() {
  ...
  for (int i = 0; i < ACTIVE_RELAYS; i++) {
    ...
    if (linkDescriptions[i] != "Brak opisu") {
      ...
      spr.setCursor(relayColX, baseY + i * lineHeight + 10);
...
void handleAutoResetLogic() {
  for (int i = 0; i < 4; i++) {
    // Jeśli wyłączone monitorowanie ALBO wyłączony globalnie AutoReset – zawsze stan IDLE, reset timera itp.
    if (!inputMonitoringEnabled[i] || !autoResetEnabled) {
      inputState[i] = IDLE;
      stateStartTime[i] = 0;
      resetAttempts[i] = 0;
      maxResetReached[i] = false;
      continue;  // nie wykonuj automatu dla tego wejścia
    }
    //Jeśli osiągnięto limit prób, pomiń dalszą obsługę
    if (inputState[i] == MAX_ATTEMPTS) {
      // Opcjonalnie: tu można zrobić obsługę ręcznego resetowania blokady
      continue;
    }
...
void handleUpdateFirmware_POST(EthernetClient &client, const String &headers, int contentLength) {
  ...
  do {
    ...
    if (Update.end(true)) {
      ...
      String updateComment = getParamValue(headers, "comment");
      if (updateComment.length()) {
        addLog("[FIRMWARE] Komentarz do aktualizacji: " + updateComment);
      }
...
void logUpdateSuccess(size_t size) {
  String logEntry = "[FIRMWARE] Aktualizacja zakończona pomyślnie. Rozmiar: " + String(size) + " bajtów";

  logEntry += ", wersja: ";
  logEntry += firmwareVersion;  // Musisz mieć np. #define firmwareVersion "v1.77"
  ...
  addLog(logEntry);  // Zapis do głównego logu log.txt

  File updateLog = SPIFFS.open("/update.log", FILE_APPEND);
  if (updateLog) {
    updateLog.println(logEntry);
    updateLog.close();
  }
}
...
// }
String getParamValue(const String &url, const String &key) {
  int pos = url.indexOf(key + "=");
  if (pos < 0) return "";
  int begin = pos + key.length() + 1;
...
void handleHttpRequest(EthernetClient &client) {
  ...
  if (requestLine.startsWith("POST /login")) {
    int cl = getContentLength(headers);
    String body = readHttpBody(client, cl);
    handleLogin_POST(client, body);
    return;
  }
  if (requestLine.startsWith("GET /logout")) {
    handleLogout(client, headers);
    return;
  }
  if (requestLine.startsWith("GET /activeSessions")) {
    sendActiveSessionsPage(client);
    return;
  }

  if (requestLine.startsWith("GET /api/mainStatus")) {
    sendApiMainStatus(client);  // zrób funkcję na JSON
    return;
  }
  

  // 2. SPRAWDZENIE SESJI (ciasteczko 'session')
  String loggedUser = checkSession(headers);
  if (loggedUser == "") {
    client.println("HTTP/1.1 302 Found");
    client.println("Location: /login");
    client.println("Connection: close");
  ...
  if (requestLine.startsWith("GET")) {
    ...
    else if (requestLine.indexOf("GET /api/log_size") != -1) {
  handleApiLogFileSize(client);
  return;
}
    ...
    if (requestLine.startsWith("POST")) {
      ...
      if (requestLine.indexOf("POST /api/AutoReset") == 0) {
        int contentLength = getContentLength(headers);
        String body = readHttpBody(client, contentLength);
      ...
      if (requestLine.indexOf("GET /toggleAuto") == 0) {
      ...
      if (requestLine.indexOf("POST /api/setInputMonitoring") == 0) {
        int contentLength = getContentLength(headers);
        String body = readHttpBody(client, contentLength);
        int inputIdx = getParamValue(body, "input").toInt();
        String en = getParamValue(body, "enabled");
        if (inputIdx >= 0 && inputIdx < 4) {
          inputMonitoringEnabled[inputIdx] = (en == "1");
          saveSystemSettings();
          client.println("HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nConnection: close\r\n\r\nOK");
        } else {
          client.println("HTTP/1.1 400 Bad Request\r\nContent-Type: text/plain\r\nConnection: close\r\n\r\nERR");
    ...
    if (requestLine.indexOf("GET /activeSessions") == 0) {
      sendActiveSessionsPage(client);
      return;
    }
    if (requestLine.indexOf("POST /logoutSession") == 0) {
      int contentLength = getContentLength(headers);
      String body = readHttpBody(client, contentLength);
      handleLogoutSession_POST(client, body);
      return;
    }
    if (requestLine.indexOf("GET /toggle_confirm?pair=") == 0) {
      int pairIdx = getParamValue(requestLine, "pair").toInt();
      if (pairIdx >= 0 && pairIdx < 2) {
        // Przełącz oba przekaźniki naraz
        int idx1 = pairIdx * 2;
        int idx2 = idx1 + 1;
        toggleRelay(idx1);
        toggleRelay(idx2);
    ...
    if (requestLine.indexOf("GET /toggle_confirm?relay=") == 0) {
      handleToggleRelayConfirm_GET(client, requestLine);
      return;
    }
    if (requestLine.indexOf("GET /togglePairLinked") == 0) {
      // 1. Parsuj numer pary (pair)
      int pairIdx = getParamValue(requestLine, "pair").toInt();
      // 2. Odwróć wartość
      relayPairLinked[pairIdx] = !relayPairLinked[pairIdx];
      // 3. Przekieruj z powrotem na główną stronę lub odśwież stronę
      client.println("HTTP/1.1 302 Found");
      client.println("Location: /");
      client.println("Connection: close");
      client.println();
    ...
    if (requestLine.indexOf("GET /toggle?relay=") == 0) {
      int relayIdx = getParamValue(requestLine, "relay").toInt();
      bool togglePair = (requestLine.indexOf("&pair=1") > 0);

      if (togglePair) {
        // Określ podstawę pary (0/1 lub 2/3)
        int pairBase = (relayIdx / 2) * 2;  // 0 albo 2
        String newState = (relayStates[pairBase] == "ON") ? "OFF" : "ON";
        relayStates[pairBase] = newState;
        relayStates[pairBase + 1] = newState;
      } else {
        relayStates[relayIdx] = (relayStates[relayIdx] == "ON") ? "OFF" : "ON";
      }
    }
  ...
  if (requestLine.startsWith("POST")) {
    ...
    if (requestLine.indexOf("POST /deleteSpiffsFile") != -1) {
    String requestBody = "";
    while (client.available()) {
        char c = client.read();
        requestBody += c;
    }
    handleDeleteSpiffsFile(client, requestBody);
    return;
}
    ...
    if (requestLine.indexOf("GET /extern bool ntpTimeSynced") == 0) {
      client.println("HTTP/1.1 200 OK");
      ...
      client.print(",\"autoResetEnabled\":");
      client.print(autoResetEnabled ? "true" : "false");
...
void handleChangeAuthPage_POST(EthernetClient &client, const String &body) {
    ...
    // ZAPISZ DO PREFERENCES (RAM nie jest już potrzebny!)
    Preferences preferences;
    preferences.begin("auth", false);
    preferences.putString("login", userPart);
    preferences.putString("password", passPart);
    preferences.end();
...
// void autoBackupLogIfNeeded(size_t maxSize = 100 * 1024) {  // funkcja przenosi plik log.txt do backupu po osiągnięciu maxSize w bajtach 10k
//   const char* logPath = "/log.txt";
//   if (!SPIFFS.exists(logPath)) return;

//   File logFile = SPIFFS.open(logPath, FILE_READ);
//   if (!logFile) return;
//   size_t size = logFile.size();
//   logFile.close();

//   if (size >= maxSize) {
//     // backup z timestampem (do minuty, żeby pliki nie nadpisywały się)
//     time_t now = time(nullptr);
//     struct tm* tm_info = localtime(&now);

//     char buf[32];
//     strftime(buf, sizeof(buf), "/log_backup_%Y%m%d_%H%M.txt", tm_info);
//     String backupName(buf);

//     // Upewnij się, że backup nie istnieje (można nadpisać, jeśli chcesz)
//     if (SPIFFS.exists(backupName)) SPIFFS.remove(backupName);

//     if (SPIFFS.rename(logPath, backupName.c_str())) {
//       File f = SPIFFS.open(logPath, FILE_WRITE);
//       if (f) f.close();
//       Serial.println("[INFO] Automatyczny backup log.txt: " + backupName);
//     } else {
//       Serial.println("[ERROR] Nie udało się utworzyć backupu log.txt");
//     }
//   }
// }
void autoBackupLogIfNeeded(size_t maxSize = 100 * 1024) {  // maxSize w bajtach (domyślnie 100 KB)
  const char* logPath = "/log.txt";
  if (!SPIFFS.exists(logPath)) return;
  ...
  if (size >= maxSize) {
    // Generuj nazwę backupu z timestampem
    time_t now = time(nullptr);
    struct tm* tm_info = localtime(&now);
    char buf[40];
    strftime(buf, sizeof(buf), "/log_backup_%Y%m%d_%H%M.txt", tm_info);
    ...
    // Jeśli backup już istnieje, usuń go
    if (SPIFFS.exists(backupName)) SPIFFS.remove(backupName);

    // Przenieś log.txt na backup
    if (SPIFFS.rename(logPath, backupName.c_str())) {
      // Utwórz nowy, pusty log.txt i dopisz komunikat systemowy
      File f = SPIFFS.open(logPath, FILE_WRITE);
      if (f) {
        String dateStr = getSafeDate(ntpConfig.getFormattedDate());
        String timeStr = getSafeTime(ntpConfig.getFormattedTime());
        String timestamp;
        if ((dateStr == "1970-01-01" && timeStr == "00:00:00") || dateStr.length() == 0 || timeStr.length() == 0) {
          timestamp = String(__DATE__) + " " + String(__TIME__);
        } else {
          timestamp = dateStr + " " + timeStr;
        }
        String msg = timestamp + " - [SYSTEM] Poprzedni plik log.txt został automatycznie zbackupowany do: " + backupName;
        f.println(msg);
        f.close();
        Serial.println(msg);
      } else {
        Serial.println("[ERROR] Nie udało się utworzyć nowego pliku log.txt po backupie!");
      }
      Serial.println("[INFO] Automatyczny backup log.txt: " + backupName);
    } else {
      String dateStr = getSafeDate(ntpConfig.getFormattedDate());
      ...
      String errMsg = timestamp + " - [ERROR] Nie udało się utworzyć backupu log.txt do: " + backupName;
      Serial.println(errMsg);
      // Możesz też dopisać błąd do log.txt, jeśli istnieje
      File ferr = SPIFFS.open(logPath, FILE_APPEND);
      if (ferr) {
        ferr.println(errMsg);
        ferr.close();
      }
...
//=================================================================================//
void autoCleanSpiffsIfNeeded(size_t maxUsed = 800 * 1024);
//======================================== wersje przeciążeniowe ADDLOG ===========//
...
void addLog(const String &eventMessage) {
  ...
  autoBackupLogIfNeeded(100 * 1024);  // <<< tutaj automatyczny backup, a nie przycinanie
   autoCleanSpiffsIfNeeded(800 * 1024); // Dodaj tę linię tutaj!
  ...
  unreadLogs = true;
  if (eventMessage.indexOf("ERROR") != -1 || eventMessage.indexOf("BŁĄD") != -1 || timestamp == String(__DATE__) + " " + String(__TIME__)) {
...
void loadSettings() {
  ...
  autoResetEnabled = preferences.getBool("autoreset", false);

  Serial.printf("LOAD: autoreset = %s\n", autoResetEnabled ? "true" : "false");
  for (int i = 0; i < 4; i++) {
    String key = "monitor" + String(i);
    inputMonitoringEnabled[i] = preferences.getBool(key.c_str(), false);
    Serial.printf("LOAD: %s = %s\n", key.c_str(), inputMonitoringEnabled[i] ? "true" : "false");
  }
  preferences.end();

  preferences.begin("stats", false);
  preferences.putString("lastLoginTime", lastLoginTime);
  preferences.putString("lastLoginIP", lastLoginIP);
  preferences.end();

  for (int i = 0; i < MAX_USERS; ++i) {
    preferences.begin("auth", true);
    String u = preferences.getString(("user" + String(i)).c_str(), userList[i]);
    String p = preferences.getString(("pass" + String(i)).c_str(), passList[i]);
    userList[i] = u;
    passList[i] = p;
    preferences.end();
  }
...
void handleToggleInput_GET(EthernetClient &client, const String &requestLine) {
  ...
  if (inputIndex >= 0 && inputIndex < 4) {
    ...
    saveSystemSettings();  // <<< Użyj swojej funkcji zapisującej WSZYSTKO!
...
void autoCleanSpiffsIfNeeded(size_t maxUsed) {
  size_t total = SPIFFS.totalBytes();
  size_t used = SPIFFS.usedBytes();

  if (used < maxUsed) return;  // nie trzeba nic robić

  // lista plików do kasowania w ustalonej kolejności
  const char* filesToTry[] = {
    "/login_history.txt",
    "/errors.log"
  };

  // zabezpieczenia – pliki których nie wolno ruszać
  ...
  const int dontTouchCount = sizeof(dontTouch)/sizeof(dontTouch[0]);

  // Funkcja pomocnicza – czy plik jest nietykalny?
  auto isDontTouch = [&](const String &fname) {
    for (int i = 0; i < dontTouchCount; ++i)
      if (fname == dontTouch[i]) return true;
    return false;
  };
  ...
  while (used > maxUsed) {
    ...
    // 1. Najpierw spróbuj kasować /login_history.txt i /errors.log
    for (size_t i = 0; i < sizeof(filesToTry)/sizeof(filesToTry[0]); i++) {
      if (SPIFFS.exists(filesToTry[i])) {
        SPIFFS.remove(filesToTry[i]);
        // Loguj do log.txt
        String msg = getCurrentDateTime() + " - [SYSTEM] Usunięto plik " + filesToTry[i] + " w celu zwolnienia miejsca w SPIFFS";
        File logf = SPIFFS.open("/log.txt", FILE_APPEND);
        if (logf) { logf.println(msg); logf.close(); }
        Serial.println(msg);
    ...
    // 2. Następnie backupy logów: /log_backup_*.txt
    File root = SPIFFS.open("/");
    File file = root.openNextFile();
    while (file) {
      String name = file.name();
      if (name.startsWith("/log_backup_") && name.endsWith(".txt")) {
        file.close();
        SPIFFS.remove(name);
        String msg = getCurrentDateTime() + " - [SYSTEM] Usunięto plik backupu " + name + " (auto-clean SPIFFS)";
        File logf = SPIFFS.open("/log.txt", FILE_APPEND);
        if (logf) { logf.println(msg); logf.close(); }
        Serial.println(msg);
        deletedSomething = true;
        used = SPIFFS.usedBytes();
        if (used <= maxUsed) return;
        // Po usunięciu przejdź od nowa (pliki mogą się przesunąć)
        break;
      }
    ...
    // 3. Jeśli nadal za dużo, kasuj "inne" pliki (ale nie nietykalne)
    root = SPIFFS.open("/");
    file = root.openNextFile();
    while (file) {
      String name = file.name();
      if (!isDontTouch(name)) {
        file.close();
        SPIFFS.remove(name);
        String msg = getCurrentDateTime() + " - [SYSTEM] Usunięto plik " + name + " (auto-clean SPIFFS)";
        File logf = SPIFFS.open("/log.txt", FILE_APPEND);
        if (logf) { logf.println(msg); logf.close(); }
        Serial.println(msg);
        deletedSomething = true;
    ...
    // Jeżeli w tej pętli nic nie skasowano, zakończ (nie ma już co kasować)
...
delay(200);
  ...
  Serial.println("[INFO] SPIFFS gotowe");

  // Automatyczna naprawa plików
  ensureAboutHtmlCurrent();

  // Tworzenie log.txt, jeśli go nie ma
  if (!SPIFFS.exists("/log.txt")) {
    File f = SPIFFS.open("/log.txt", FILE_WRITE);
    f.close();
    Serial.println("Utworzono nowy plik log.txt");
  }
  ...
  // Licznik restartów
  lastRestartEpoch = time(nullptr); // pobiera bieżący czas z RTC/NTP//1
  preferences.begin("stats", false);
  preferences.putULong("lastRestartEpoch", lastRestartEpoch);//2
...
String macStr = preferences.getString("mac_str", "02:50:57:01:01:75");
telnetPort = preferences.getInt("telnetPort", 23); // domyślnie 23
preferences.end();
...
if (macStr.length() == 17) {
  ...
  for (int i = 0; i < maxAttempts; i++) {
    ...
    // Ustawienie trasy statycznej
    //secureServer.serveStatic("/about.html", SPIFFS, "/about.html");
  ...
  addLog("[SYSTEM] Start systemu – " + getCurrentDateTime());

  Serial.println(F("\n================= DIAGNOSTYKA STARTU ================="));

Serial.println(F("--- SIEC ---"));
Serial.printf("Urządzenie: %s-%d\nMAC: %02X:%02X:%02X:%02X:%02X:%02X\nIP: %s\n",
  PRODUCT_NAME, DEVICE_NUMBER,
  mac[0], mac[1], mac[2], mac[3], mac[4], mac[5],
  ip.toString().c_str());
Serial.print(F("Gateway: ")); Serial.println(gatewayStr);
Serial.print(F("Subnet: ")); Serial.println(subnetStr);
Serial.print(F("DNS: ")); Serial.println(dnsStr);

Serial.println(F("--- AUTORYZACJA ---"));

Serial.println("Loaded WWW login: " + httpUser);
Serial.println("Loaded WWW pass: " + httpPass);

//Serial.println("[AUTH] Próba logowania: " + podanyLogin + "/" + podaneHaslo);

// Serial.println(F("--- AUTORYZACJA ---"));
// Serial.print(F("Login: ")); Serial.println(httpUser);
// Serial.print(F("Hasło: ")); Serial.println(httpPass);

Serial.println(F("--- KONFIGURACJA CZASÓW I RELAY ---"));
Serial.print(F("Liczba przekaźników: ")); Serial.println(ACTIVE_RELAYS);
Serial.print(F("Czas RESET_DURATION [ms]: ")); Serial.println(RESET_DURATION);
Serial.print(F("Czas DEVICE_STARTTIME [ms]: ")); Serial.println(DEVICE_STARTTIME);
Serial.print(F("Czas WAITING_1MIN_TIME [ms]: ")); Serial.println(WAITING_1MIN_TIME);
Serial.print(F("Czas autoOffTime [ms]: ")); Serial.println(autoOffTime);
...
Serial.print(F("Strefa czasowa: ")); Serial.println(timeZone);
Serial.print(F("Offset (sek): ")); Serial.println(timezoneOffset);
Serial.print(F("Serwer NTP 1: ")); Serial.println(ntpServer1);
Serial.print(F("Serwer NTP 2: ")); Serial.println(ntpServer4);
Serial.print(F("Serwer NTP 3: ")); Serial.println(ntpServer3);
Serial.print(F("NTP (adres): ")); Serial.println(ntpServerAddress);
Serial.print(F("NTP (port): ")); Serial.println(ntpServerPort);

Serial.println(F("--- INNE ---"));
Serial.print(F("placeStr: ")); Serial.println(placeStr);
Serial.printf("Ostatni restart (epoch): %lu\n", lastRestartEpoch);
Serial.printf("Liczba restartów: %lu\n", deviceRestarts);

// Możesz dodać:
// Serial.print(F("Wersja firmware: ")); Serial.println(firmwareVersion);
 Serial.printf("Free heap: %lu bytes\n", ESP.getFreeHeap());

Serial.println();
Serial.println(F("--- LISTA PLIKÓW SPIFFS ---"));
listSpiffsFiles();
...
void sendCommonHtmlHeader(EthernetClient &client, const String &title) {
  ...
  client.println("<div id='logoutTimer' style='color:#ffa000; font-size:1em; font-weight:bold; margin-bottom:18px;'></div>");

  client.println("<div style='margin:12px 0 22px 0; text-align:right;'>"
                 "<a href=\"/activeSessions\" style='color:#4fc3f7;font-weight:bold;'>Zalogowani użytkownicy</a>"
                 "</div>");
  // --- Dodaj separator i przycisk Wyloguj na dole ---
  client.println("<hr style='margin:16px 0; border-color:#303648;'>");
  client.println("<a href='/logout' style='color:#ff5252; font-weight:bold; letter-spacing:1px;'>Wyloguj</a>");
  client.println("</div>");

  client.println("<script>");
  client.println("let logoutTimeout = null;");
  client.println("const AUTO_LOGOUT_MS = 30*60*1000;");  // 30 minut
  client.println("let timeLeft = AUTO_LOGOUT_MS / 1000;");
  client.println("function resetAutoLogout() {");
  client.println("  timeLeft = AUTO_LOGOUT_MS / 1000;");
  client.println("  if (logoutTimeout) clearTimeout(logoutTimeout);");
  client.println("  logoutTimeout = setTimeout(function() {");
  client.println("    alert('Z powodu bezczynności nastąpi automatyczne wylogowanie.');");
  client.println("    location.href = '/logout';");
  client.println("  }, AUTO_LOGOUT_MS);");
  client.println("}");
  client.println("function showTimer() {");
  client.println("  let min = Math.floor(timeLeft / 60);");
  client.println("  let sec = (timeLeft % 60).toString().padStart(2, '0');");
  client.println("  const el = document.getElementById('logoutTimer');");
  client.println("  if (el) el.innerText = 'Wylogowanie za: ' + min + 'm ' + sec + 's';");
  client.println("  if(timeLeft > 0) { timeLeft--; setTimeout(showTimer, 1000); }");
  client.println("}");
  client.println("['click', 'mousemove', 'keydown', 'touchstart'].forEach(ev => document.addEventListener(ev, resetAutoLogout));");
  client.println("resetAutoLogout();");
  client.println("showTimer();");
  client.println("</script>");
...
void sendDiagnosticPage(EthernetClient &client) {
  ...
  // --- AKTUALNY CZAS na górze pod nagłówkiem ---
    // --- Panel z czasem, synchronizacją NTP, przyciskiem i wersją ---
client.println("<div style='margin:18px 0 0 0; background:#262d39; padding:10px 18px; border-radius:8px; color:#fff;'>");

String currentDateTime = getCurrentDateTime(); // Twoja funkcja zwracająca sformatowany czas
client.print("<span id='currentTime' style='color:#4fc3f7;'>Aktualny czas: ");
client.print(currentDateTime);
client.println("</span>");

// Wyświetl informację o synchronizacji NTP lub brak synchronizacji
if (lastSyncEpochTime > 0 && lastSyncEpochTime > 1700000000) { // np. po 2023 roku
    String lastSyncStr = formatEpochTime(lastSyncEpochTime);    // Twoja funkcja
    client.println("<br><span style='color:#b2dfdb;'>Ostatnia synchronizacja NTP: " + lastSyncStr + "</span>");
} else {
    client.println("<br><span style='color:#ff5252; font-weight:bold;'>Brak synchronizacji NTP!</span>");
}
...
// Wersja firmware
client.println("<span style='margin-left:30px; color:#bdbdbd; font-size:0.92em;'>"
               "Wersja: v<span id='firmwareVer'>" + String(firmwareVersion) + "</span></span>");
...
client.println("<tr style='background:#2c323c;'><th style='padding:8px;'>Nazwa pliku</th><th style='padding:8px;'>Rozmiar</th><th style='padding:8px;'>Podgląd</th><th style='padding:8px;'>Pobierz</th><th style='padding:8px;'>Usuń</th><th style='padding:8px;'>Zmień nazwę</th></tr>");
...
while (file) {
  ...
  if (name.indexOf("log_backup_") >= 0 && name.endsWith(".txt")) {
    ...
    client.println("</td>");
    // Rozmiar pliku
    client.print("<td style='padding:8px; text-align:right; color:#ffd600;'>");
    client.print(file.size());
    client.println(" B</td>");
    // Podgląd jako przycisk
    client.print("<td style='padding:8px; text-align:center;'>");
    client.print("<form method='GET' action='/viewLogBackup' style='display:inline;'>");
    client.print("<input type='hidden' name='file' value=\"" + name + "\">");
    client.print("<button type='submit' style='background:#2196f3;color:#fff;border:none;border-radius:4px;padding:6px 14px;cursor:pointer;'>Podgląd</button>");
    client.print("</form>");
    client.println("</td>");
    // Pobierz jako przycisk
    client.print("<td style='padding:8px; text-align:center;'>");
    client.print("<form method='GET' action='/downloadLogBackup' style='display:inline;'>");
    client.print("<input type='hidden' name='file' value=\"" + name + "\">");
    client.print("<button type='submit' style='background:#25cc69;color:#fff;border:none;border-radius:4px;padding:6px 14px;cursor:pointer;'>Pobierz</button>");
    client.print("</form>");
    client.println("</td>");
    // Usuń jako przycisk
    client.print("<td style='padding:8px; text-align:center;'>");
...
// --- Dodaj pliki główne logów z rozmiarem ---
...
for (int i = 0; i < 3; i++) {
  ...
  if (SPIFFS.exists(name)) {
    ...
    client.print("<td style='padding:8px; text-align:center;'>");
    client.print("<form method='GET' action='/viewLogBackup' style='display:inline;'>");
    client.print("<input type='hidden' name='file' value=\"" + name.substring(1) + "\">");
    client.print("<button type='submit' style='background:#2196f3;color:#fff;border:none;border-radius:4px;padding:6px 14px;cursor:pointer;'>Podgląd</button>");
    client.print("</form>");
    client.println("</td>");
    // Pobierz jako przycisk
...
if (!found) {
  client.println("<tr><td colspan='6' style='padding:18px;text-align:center;color:#aaa;'>Brak kopii logów.</td></tr>");
}
client.println("</table>");
client.println("</div>");
sendCommonHtmlFooter(client);
...
void saveSystemSettings() {
  preferences.begin("system", false);  // tryb zapisu!
  preferences.putBool("autoreset", autoResetEnabled);

  for (int i = 0; i < 4; i++) {
    String key = "monitor" + String(i);
    preferences.putBool(key.c_str(), inputMonitoringEnabled[i]);
    Serial.printf("SAVE: %s = %s\n", key.c_str(), inputMonitoringEnabled[i] ? "true" : "false");
    Serial.printf("SAVE: autoreset = %s\n", autoResetEnabled ? "true" : "false");
  }
  preferences.end();
}

void sendApiMainStatus(EthernetClient &client) {
  client.println("HTTP/1.1 200 OK");
  client.println("Content-Type: application/json");
  ...
  client.print(",\"placeStr\":\"" + escapeJson(placeStr) + "\"");
  String _currentTime = getCurrentDateTime();
  if (_currentTime == "brak synchronizacji" || _currentTime == "" || _currentTime == "N/A") {
#ifdef buildDateTime
    _currentTime = String("Kompilacja: ") + buildDateTime;
#else
    _currentTime = String("Kompilacja: ") + String(__DATE__) + " " + String(__TIME__);
#endif
  }
  client.print(",\"currentTime\":\"" + escapeJson(_currentTime) + "\"");
...
void listSpiffsFiles() {
  Serial.println("=== Zawartość SPIFFS ===");
  File root = SPIFFS.open("/");
  if (!root || !root.isDirectory()) {
    Serial.println("[ERROR] Nie można otworzyć katalogu głównego SPIFFS!");
    return;
  }

  File file = root.openNextFile();
  while (file) {
    Serial.printf("Plik: %-30s | Rozmiar: %8d B\n", file.name(), file.size());
    file = root.openNextFile();
  }
  Serial.println("=========================");
}
...
//funkcja sprawdzająca plik about.html w SPIFFS
void ensureAboutHtml() {
  const char *path = "/about.html";
  const uint8_t *buf = (const uint8_t *)about_html;
  size_t buflen = strlen(about_html);

  if (SPIFFS.exists(path) && spiffsFileMatches(path, buf, buflen)) {
    Serial.println("[INFO] Plik /about.html aktualny – nie nadpisuję");
    return;
  }
  ...
  if (f) {
    f.write(buf, buflen);
    f.close();
    Serial.println("[INFO] Nadpisano plik /about.html (nowa wersja wgrana do SPIFFS)");
  } else {
    Serial.println("[ERROR] Nie udało się zapisać /about.html!");
  }
}

void handleAboutPage(EthernetClient &client) {
  if (SPIFFS.exists("/about.html")) {
    File f = SPIFFS.open("/about.html", FILE_READ);
    if (f) {
      // Wyślij nagłówki HTTP
      client.println("HTTP/1.1 200 OK");
      client.println("Content-Type: text/html; charset=UTF-8");
      client.println();
  ...
  // Jeżeli pliku NIE MA lub błąd: wyślij z PROGMEM
  client.println("HTTP/1.1 200 OK");
  client.println("Content-Type: text/html; charset=UTF-8");
  client.println();
  client.print(FPSTR(about_html));
}


void handleAboutSmart(EthernetClient &client) {
  if (SPIFFS.exists("/about.html")) {
    // Wyślij plik z SPIFFS
    File file = SPIFFS.open("/about.html", FILE_READ);
    if (!file) {
      client.println("HTTP/1.1 500 Internal Server Error");
      client.println("Content-Type: text/html");
      client.println();
      client.println("<html><body><h2>Błąd otwarcia about.html</h2></body></html>");
      return;
    }
    client.println("HTTP/1.1 200 OK");
    client.println("Content-Type: text/html");
    ...
    // Wygeneruj stronę z funkcji
    sendAboutPage(client);
  }
}


// Prosty CRC32 (polinom 0xEDB88320)
...
bool spiffsFileMatches(const char *path, const uint8_t *buf, size_t buflen) {
  ...
  // Możesz wczytać plik do bufora i porównać CRC lub bajt po bajcie
  uint32_t crcFile = 0xFFFFFFFF;
  while (f.available()) {
    uint8_t c = f.read();
    crcFile ^= c;
    for (int k = 0; k < 8; k++)
      crcFile = crcFile & 1 ? (crcFile >> 1) ^ 0xEDB88320 : crcFile >> 1;
  }
  crcFile = ~crcFile;
  uint32_t crcBuf = crc32(buf, buflen);

  f.close();
  return (crcFile == crcBuf);
}

void ensureAboutHtmlCurrent() {
  if (!spiffsFileMatches("/about.html", (const uint8_t *)about_html, strlen(about_html))) {
    File f = SPIFFS.open("/about.html", FILE_WRITE);
    if (f) {
      f.write((const uint8_t *)about_html, strlen(about_html));
      f.close();
      Serial.println("[INFO] Plik /about.html utworzony lub zaktualizowany z PROGMEM");
    } else {
      Serial.println("[ERROR] Nie udało się zapisać /about.html!");
    }
  } else {
    Serial.println("[INFO] Plik /about.html w SPIFFS jest aktualny");
  }
}

void handleApiLogFileSize(EthernetClient &client) {
  File f = SPIFFS.open("/log.txt", FILE_READ);
  size_t size = f ? f.size() : 0;
  if (f) f.close();
  client.println("HTTP/1.1 200 OK");
...
void logSystemEventToLog(const String &msg) {
  String dateStr = getSafeDate(ntpConfig.getFormattedDate());
  String timeStr = getSafeTime(ntpConfig.getFormattedTime());
  String timestamp;

  // Jeśli nie ma synchronizacji NTP, użyj daty kompilacji
  if ((dateStr == "1970-01-01" && timeStr == "00:00:00") || dateStr.length() == 0 || timeStr.length() == 0) {
    timestamp = String(__DATE__) + " " + String(__TIME__);
  } else {
    timestamp = dateStr + " " + timeStr;
  ...
  if (f) {
    f.println(timestamp + " - [SYSTEM] " + msg);
    f.close();
  }
}
